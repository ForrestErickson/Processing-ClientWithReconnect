 ...
 * 15 July. Change isconnected to boolean. Update screen prompt when trying to get socket.
...
Client myClient;
final int reconnectPeriod = 3500;  // 3.5 Seconds
...
void getSocket() {
  /// Tries to open a client network socket connection.
    try {                                // Might through an IOException when connecting to socket.
      ...
      text(("Exception looking for Socket. I got npe= "+npe), 400, 10);
...
void draw() {
  ...
  } else if (timeReconnect +reconnectPeriod >millis()) {          // Not isconnected
    //Update window
    myBackground = color(0, 64, 0);
